...
RUN_ID="${1:-hf-head-deprel-eval-001}"
MODEL="${MODEL:-Qwen/Qwen2.5-0.5B-Instruct}"
ADAPTER_PATH="${ADAPTER_PATH-hf_outputs/qwen25-05b-head-deprel-a100-lora-full}"
SYSTEM_NAME="${SYSTEM_NAME:-hf_lora_head_deprel}"
TEST_FILE="${TEST_FILE:-latin_head_deprel_data/test.jsonl}"
...
python ../latin_lora_mlx_ft/scripts/extract_gold_conllu_from_mlx_jsonl.py \
  "$TEST_FILE" \
  "$RUN_DIR/gold.conllu" \
  $LIMIT_ARGS
...
if [ -n "$ADAPTER_PATH" ]; then
  python scripts/generate_hf_head_deprel_predictions.py \
    --input "$TEST_FILE" \
    --output "$RUN_DIR/head_deprel_predictions.jsonl" \
    --model "$MODEL" \
...
if ! python scripts/render_head_deprel_to_conllu.py \
    --input-jsonl "$TEST_FILE" \
    --pred-jsonl "$RUN_DIR/head_deprel_predictions.jsonl" \
