...
SYSTEM_PROMPT = (
    "Predict Latin dependencies. Given blank CoNLL-U and # token_count, output "
    "exactly that many ID<TAB>HEAD<TAB>DEPREL rows, then END."
)
...
def with_token_count(conllu, token_count):
    lines = conllu.splitlines()
    insert_at = 0
    while insert_at < len(lines) and lines[insert_at].startswith("#"):
        insert_at += 1
    lines.insert(insert_at, f"# token_count = {token_count}")
    return "\n".join(lines)
...
def convert_record(record, system_prompt):
    ...
    return {
        "messages": [
            ...
            {"role": "user", "content": with_token_count(user_conllu, len(user_rows))},
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert full CoNLL-U chat JSONL into sentence-level ID/HEAD/DEPREL+END chat JSONL."
    )
