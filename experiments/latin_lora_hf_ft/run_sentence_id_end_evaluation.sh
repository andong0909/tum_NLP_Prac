...
RUN_ID="${1:-hf-sentence-id-head-deprel-end-eval-001}"
MODEL="${MODEL:-Qwen/Qwen2.5-0.5B-Instruct}"
ADAPTER_PATH="${ADAPTER_PATH-hf_outputs/qwen25-05b-sentence-id-head-deprel-end-a100-lora-full}"
SYSTEM_NAME="${SYSTEM_NAME:-hf_lora_sentence_id_head_deprel_end}"
TEST_FILE="${TEST_FILE:-latin_sentence_id_head_deprel_end_data/test.jsonl}"
...
python - "$TEST_FILE" "$RUN_DIR/gold_id_head_deprel_predictions.jsonl" $LIMIT_ARGS <<'PY'
import argparse
import json
...
with args.input_jsonl.open(encoding="utf-8") as handle, args.output_jsonl.open("w", encoding="utf-8") as out:
    for line in handle:
        ...
        out.write(json.dumps({"assistant": record["messages"][2]["content"]}, ensure_ascii=False) + "\n")
        count += 1
        if args.limit is not None and count >= args.limit:
            break
PY

python scripts/render_sentence_id_head_deprel_end_to_conllu.py \
  --input-jsonl "$TEST_FILE" \
  --pred-jsonl "$RUN_DIR/gold_id_head_deprel_predictions.jsonl" \
...
if [ -n "$ADAPTER_PATH" ]; then
  python scripts/generate_hf_head_deprel_predictions.py \
    --input "$TEST_FILE" \
    --output "$RUN_DIR/id_head_deprel_predictions.jsonl" \
    --model "$MODEL" \
...
if ! python scripts/render_sentence_id_head_deprel_end_to_conllu.py \
    --input-jsonl "$TEST_FILE" \
    --pred-jsonl "$RUN_DIR/id_head_deprel_predictions.jsonl" \
